...
# here, we examine the setting where the the response function is 
# a random neural network applied to the input covariates z
...
parser.add_argument('--gamma_end', type=float, default=1.5)
parser.add_argument('--n_ps', type=int, default=15, help='number of values of p to test (+/-)')
parser.add_argument('--response_nn_layers', type=lambda x: [int(y) for y in x.split(',')],
    default=[128], help='architecture of random neural network')
parser.add_argument('--response_nn_activation', type=str, default='tanh')
...
response_nn = tf.keras.Sequential(
    [
    tf.keras.layers.Dense(n_neurons, activation=args.response_nn_activation)
    for n_neurons in args.response_nn_layers] \
    + [tf.keras.layers.Dense(1, activation='linear')])

epsilon = np.random.normal(loc=0, scale=nu, size=(n,1))
y = response_nn(Z) + epsilon

# save random neural network response func
response_nn.save(f'results/{out_dir}/response_nn.h5')


# print data schema
print(f'n = {n}, n_train = {n_train}, d = {d}, lambda = {lamda}')
